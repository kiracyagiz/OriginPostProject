...
const FeaturesCard = ({ ImagepositionLeft, imagesrc,
h1,h2,p,primary1,primary2,
}) => {
  return (
    <div>

        <div className={`flex flex-col ${ImagepositionLeft ? 'md:flex-row' : 'md:flex-row-reverse'} md:mx-72 gap-x-4  items-center  justify-evenly px-4 md:p-0 gap-y-8   mt-8 md:mt-20`}>
          <div>
            <Image src={imagesrc} className="rounded-lg object-fill shadow-2xl hover:scale-105 hover:shadow-md cursor-pointer hover:shadow-black  transition-transform duration-300"></Image>
          </div>
          <div className="flex flex-col ">
            <p className="tracking-tighter">{h2}</p>
            <p className="text-xl font-bold  max-w-sm  mb-6">{h1}</p>
