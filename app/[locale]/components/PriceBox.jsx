...
const PriceBox = ({ feature, price, desc }) => {
  ...
  return (
    <div className="w-70 h-405 mt-8 bg-white border border-gray-500 rounded-lg mx-auto p-4 text-center flex flex-col justify-between items-center hover:scale-105 hover:shadow-md cursor-pointer hover:shadow-black  transition-transform duration-300">
      <h3 className="font-bold text-lg mt-2 border-b-2 border-b-gray-500 w-full pb-2">
        {feature}
      </h3>

      <h3 className="text-lg text-primaryYellow font-bold">{price}</h3>
      <p className="text-lg font-bold">{t("desc")}</p>
      ...
      <ButtonPrimary text={t("priceButton")} />
    </div>
  );
};
